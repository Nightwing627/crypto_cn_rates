...
const multi_crypto_buy = multiply['Cad']['Crypto_buy']
const multi_crypto_sell = multiply['Cad']['Crypto_sell']

const multi_currency_buy = multiply['Cad']['Currency_buy']
const multi_currency_sell = multiply['Cad']['CUrrency_sell']
...
export default class Component extends React.Component {
  ...
  componentDidMount() {
    document.title = "Coin Nerds Canada Rates"
    setInterval(() => {
      axios.get(`getconversion1`)
      .then(async (res) => {
        if (1) {
          this.setState({
            server: true
